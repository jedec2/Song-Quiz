...
// ==================================================
// I. CORE UTILITIES & UI UPDATES (GLOBAL)
// ==================================================
...
function resetRound(){
    ...
    activePlayer = 'user';
    buzzLocked = false;
    inputZone.classList.remove("vis","suc","fail");
...
// ==================================================
// II. GAME ACTIONS AND LOGIC (GLOBAL)
// ==================================================
...
async function submitAnswer(){
    ...
    try{
        ...
        if(hits&&hits.length>0&&hits[0].type==="song"){
            ...
            // Logic to check length ratio or simple overlap logic
...
// ==================================================
// III. GAME MODE SETUP AND FLOW CONTROL (GLOBAL)
// ==================================================

function initializeGame(mode, timeLimit) {
    clearInterval(timerInterval);
    gameState.currentMode = mode;
    // Reset UI visibility for setup
    mainGame.style.display='none';
    winScreen.classList.remove("vis");

    // Ensure 'setup-mode' container is visible and ready
    const setupContainer = document.getElementById('setup-mode');
    if(setupContainer) {
...
function startGame(mode, timeLimit) {
    ...
    const inputName = $("name-p1");
    if(inputName && inputName.value.trim()) {
        gameState.name = inputName.value.trim();
        labelP1.textContent = gameState.name;
    }

    // Ensure score and rounds reset
    gameState.score = 0;
    gameState.winTarget = parseInt($("win-target")?.value) || 5;
    timedChallengeState.totalRounds = 0;

    // Reset UI Visibility: Hide Name Setup, Show Main Game
...
// Attach manual clickable event triggers (GLOBAL)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeGame(mode, timeLimit) {
        ...
        // Reset name/target display logic
        ...
        if (winRow) {
            ...
            if (mode === 'timed') {
                winRow.innerHTML += `<label>Time Limit</label><input id="round-limit" type="number" min="1" value="10" />`;
            }
        }

        // Make the name setup visible, but hide the actual game elements
        const nameSetupEl = document.getElementById('name-setup');
        if(nameSetupEl) nameSetupEl.style.display = "block";

        const mainGameEl = document.getElementById('main-game');
        if(mainGameEl) mainGameEl.style.display = "none"; 
    }
    ...
    if (modeContainer) {
        modeContainer.addEventListener('click', (e) => {
            ...
            if (selectedMode === 'timed') {
                ...
                timeLimitValue = roundInput ? parseInt(roundInput.value) : 10;
            } else {
    ...
    if (startGameBtn) {
        startGameBtn.addEventListener("click", () => {
            ...
            // Read inputs from DOM directly to ensure consistency with HTML values
            const winTargetInput = document.getElementById('win-target');
            const timeLimitInput = document.getElementById('round-limit');

            if (finalMode) {
                startGame(finalMode, timeLimitInput ? parseInt(timeLimitInput.value) : 0);
            } else {
